...
public class VTurtleArmorItem extends Item {
    ...
    private static final Map<ArmorMaterial, List<StatusEffectInstance>> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, List<StatusEffectInstance>>())
                    .put(AQArmorMaterials.AQ_TURTLE_INSTANCE,
                            List.of(new StatusEffectInstance(StatusEffects.WATER_BREATHING, effectDurationInSeconds, effectAmplifier, isEffectAmbient, isEffectVisible))).build();

    public VTurtleArmorItem(Settings settings) {
        super(settings);
    }
    ...
    private boolean hasCorrectArmorOn(PlayerEntity p) {
        return p.getEquippedStack(EquipmentSlot.HEAD).isOf(Items.TURTLE_HELMET)
                && p.getEquippedStack(EquipmentSlot.CHEST).isOf(AQItems.VTURTLE_CHESTPLATE)
                && p.getEquippedStack(EquipmentSlot.LEGS).isOf(AQItems.VTURTLE_LEGGINGS)
                && p.getEquippedStack(EquipmentSlot.FEET).isOf(AQItems.VTURTLE_BOOTS);
    }
}
